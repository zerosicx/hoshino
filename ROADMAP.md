...
- [x] **Marketing site** — `docs/` on GitHub Pages: a landing page with a
      live demo of the dictionary over 17,673 common and JLPT words
      (`docs/demo/words.json`, regenerated by `scripts/build-site-demo.ts`),
      an About page with the founder's note, privacy and source credits, and
      the standalone privacy policy. Hannah switches Pages on (main, /docs);
      the Play Store button is a placeholder until the listing exists. A
      feature-requests page comes later, as an anonymous channel.
- [ ] **Web app** — `npx expo export --platform web`, deploy with the headers
      above. Not GitHub Pages: it cannot set COOP/COEP, which is why the site
      carries a static demo rather than the app itself.
